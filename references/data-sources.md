...
| 错误 | 处理 |
|------|------|
| HTTP 429 `TOO_MANY_REQUESTS` | 等待下一个小时窗口后重试 |
| `COUNT_EXCEEDS_MAX` | count 降到 20（这是上限，不是 schema 写的 200） |
| HTTP 5xx | 退避 2s 重试，最多 3 次 |
| 空响应 | 分页到底，正常停止 |
...
- **total_number 与实际可取回数不符**：如 total=763 但去重后只能收敛到 640——缺口通常是注销/私密/被封/不可见账号，API 计入 total 但永远不返回。
- **cursor 链断裂**：链式翻页中途返回空段，直接跳后段又有数据。
- **排序漂移**：翻页间列表重排，页间重叠（每页 20 只新增 19/16/9……）。
